...
{
    auto mid = (aP0 + aP1) * 0.5;
    auto chord = aP1 - aP0;
    auto c = (aP1 - aP0).EuclideanNorm() / 2;
    auto d = chord.Perpendicular();

    m_pc = mid + d.Resize( (double) c / tan( aCenterAngle / 2.0 * M_PI / 180.0 ) );
    m_p0 = aP0;
    m_p1 = aP1;
        ...
        double aRadius )
{
    ...
    auto d2 = VECTOR2D( 1.0, 0.0 ).Rotate( (aStartAngle + aCenterAngle) * M_PI / 180.0 ) * aRadius;
