...
void VIEW_BASE::AddTree( int aId )
{
    ...
    {
        m_trees[aId] = new VIEW_RTREE();
    }
}



void VIEW_BASE::Add( VIEW_ITEM* aItem, int aLayer )
{
    int layers[ 16 ];
    int count;
    ...
    {
        aItem->ViewGetLayers ( layers, count );
        if (count)
            aLayer = layers[0];
    }
    ...
    VIEW_RTREE *rtree = m_trees [aLayer];
    ...
    //printf(" add bbox %s\n", aItem->ViewBBox().Format().c_str() );

    //printf("pre cnt %d\n", rtree->Count() );
    rtree->Insert( rtreeEnt, aItem->ViewBBox() );
    //printf("post cnt %d\n", rtree->Count() );
...
void VIEW_BASE::Remove( VIEW_ITEM* aItem )
...
void VIEW_BASE::Update ( VIEW_ITEM* aItem )
{
//    SetDirty( aItem, true );
...
VECTOR2D VIEW_BASE::ToWorld( const VECTOR2D& aCoord, bool aAbsolute ) const
{
...
double VIEW_BASE::ToWorld( double aSize ) const
{
    const MATRIX3x3D& matrix = m_gal->GetScreenWorldMatrix();
...
VECTOR2D VIEW_BASE::ToScreen( const VECTOR2D& aCoord, bool aAbsolute ) const
{
...
double VIEW_BASE::ToScreen( double aSize ) const
{
    const MATRIX3x3D& matrix = m_gal->GetWorldScreenMatrix();
...
void VIEW_BASE::SetGAL( GAL* aGal )
{
    m_gal = aGal;
...
BOX2D VIEW_BASE::GetViewport() const
{
    BOX2D    rect;
...
void VIEW_BASE::SetViewport( const BOX2D& aViewport )
{
...
void VIEW_BASE::SetMirror( bool aMirrorX, bool aMirrorY )
{
    m_gal->SetFlip( aMirrorX, aMirrorY );
}


void VIEW_BASE::SetScale( double aScale, const VECTOR2D& aAnchor )
{
...
void VIEW_BASE::SetCenter( const VECTOR2D& aCenter )
{
    m_center = aCenter;
...
void VIEW_BASE::SortLayers( int aLayers[], int& aCount ) const
{
...
const VECTOR2I& VIEW_BASE::GetScreenPixelSize() const
{
...
void VIEW_BASE::clearGroupCache()
{
  printf("clear-group-cache\n");
    #if 0
    BOX2I r;
...
struct VIEW_BASE::extentsVisitor
{
    BOX2I extents;
    bool first;
    int items;
    ...
    bool operator()( VIEW_RTREE_ENTRY* aItem )
    {
        ...
        if( first )
        {
            extents = aItem->item->ViewBBox();
            first = false;
        }
        else
            extents.Merge ( aItem->item->ViewBBox() );
...
const BOX2I VIEW_BASE::CalculateExtents()
{
    extentsVisitor v;
    BOX2I fullScene;
    fullScene.SetMaximum();

    BOOST_FOREACH( VIEW_RTREE *tree, m_trees| boost::adaptors::map_values )
    {
        ...
        if( aLayer->cacheIndex >= 0 && ent->TestFlags( VI_CACHEABLE ) )
        {
            ...
            if( ent->IsDirty() || grp < 0 )
            {
                ...
                if ( !m_painter->Draw( item, aLayer->painterLayer ) )
                    item->ViewDraw ( aLayer->painterLayer, this );
...
VIEW_BASE::RENDER_LAYER* VIEW_BASE::addLayer(
        ...
        int aCacheIndex,
        std::function<int (VIEW_ITEM*)> aLODFunc )
{
...
{
    BOOST_FOREACH(VIEW_RTREE *rtree, m_trees | boost::adaptors::map_values)
    {
        rtree->RemoveAll();
    }
...
void VIEW_BASE::SetVisible( const VIEW_ITEM *aItem, bool aShow )
{

}

void VIEW_BASE::Hide ( const VIEW_ITEM *aItem, bool aShow )
{

}

bool VIEW_BASE::IsVisible ( const VIEW_ITEM *aItem) const
{
    return true;
}
