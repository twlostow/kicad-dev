...
private:
    ...
    bool isTheConstraint(const DartType& dart, const DartType& dstart, const DartType& dend) 
    {
        DartType d0 = dart;
        d0.Alpha0(); // CW
        if ((Same0Orbit(dstart, dart) && Same0Orbit(dend, d0)) ||
        (Same0Orbit(dstart, d0)  && Same0Orbit(dend, dart)))
        {
            return true;
        }
  ...
  template <class TraitsType, class DartType>
    ...
    if (IsBoundaryNode(d_iter)) {
      d_iter.Alpha1(); // CW
      PositionAtNextBoundaryEdge(d_iter); // CCW (was rotated CW to the boundary)
    }
  ...
  template <class TraitsType, class DartType>
    ...
    do {
      ...
      while(it != elist.end()) { // ??? change this test with counter > dartsInList
        ...
        if (SwappableEdge<TraitsType, DartType>(*it, true)) {
                                           ...
                                           DART_LIST_TYPE& aElist )
{
  ...
  */
template <class TraitsType, class DartType>
DartType TRIANGULATION_HELPER::InsertConstraint(DartType& dstart, DartType& dend, bool optimize_delaunay) {
    ...
    DartType next_start = this->findCrossingEdges<TraitsType>(dstart, dend, elist);
    ...
    if (elist.size() > 0) // by Thomas Sevaldrud
       transformToConstraint<TraitsType>(dstart, next_start, elist);
