...
void BOARD_COMMIT::Push( const wxString& aMessage )
{
    ...
    if ( editModules )
    {
        frame->SaveCopyInUndoList(  board->m_Modules, UR_MODEDIT );
    }
    ...
    BOOST_FOREACH( COMMIT_LINE& ent, m_changes )
    {
        switch(ent.m_type)
        {
            case CHT_ADD:
            {
                if( !editModules )
                {
                    ITEM_PICKER itemWrapper( ent.m_item, UR_NEW );
                    undoList.PushItem( itemWrapper );
                    board->Add( static_cast<BOARD_ITEM*> (ent.m_item) );
                } else {
                    board->m_Modules->Add( static_cast<BOARD_ITEM*> (ent.m_item) );
                }

                if(ent.m_item->Type() == PCB_MODULE_T )
                {
                    MODULE *mod = (MODULE *) ent.m_item;
            ...
            case CHT_REMOVE:
            {
                if ( !editModules )
                {
                    ITEM_PICKER itemWrapper( ent.m_item, UR_DELETED );
                    undoList.PushItem( itemWrapper );
                    board->Remove(static_cast<BOARD_ITEM*> ( ent.m_item) );
                } else {
                    board->m_Modules->Remove( static_cast<BOARD_ITEM*> (ent.m_item) );
                }
            ...
            case CHT_MODIFY:
            {
                ...
                ent.m_item->ViewUpdate ( KIGFX::VIEW_ITEM::ALL );

                ratsnest->Update( static_cast<BOARD_ITEM*> ( ent.m_item  ) );
