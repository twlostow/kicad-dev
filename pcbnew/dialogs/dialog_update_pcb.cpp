    ...
    m_netlist (aNetlist)
{
    ...
    m_messagePanel->SetVisibleSeverities( REPORTER::WARNING | REPORTER::ERROR | REPORTER::ACTION );
}
...
void DIALOG_UPDATE_PCB::PerformUpdate( bool aDryRun )
{
    ...
    catch ( IO_ERROR &error )
    {
        ...
        reporter.Report ( _("Failed to load one or more footprints. Please add the missing libraries in PCBNew configuration. The PCB will not update completely."), REPORTER::ERROR );
        reporter.Report ( error.errorText, REPORTER::INFO );
    }
