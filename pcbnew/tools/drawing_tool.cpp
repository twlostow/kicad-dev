...
TOOL_ACTION linearConstraintOrientation( "pcbnew.InteractiveDrawing.linearConstraintOrientation", AS_CONTEXT,
            ' ',
...
DRAWING_TOOL::DRAWING_TOOL() :
    ...
    m_menu( *this )
{
    m_outlineShapeType = OUTLINE_SHAPE_BUILDER::SHT_LINE;
...
bool DRAWING_TOOL::Init()
{
    ...
    ctxMenu.AddItem( linearConstraintOrientation, drawingLinearConstraint, 1000 );
...
const std::vector<DRAWSEGMENT*> DRAWING_TOOL::convertShapesToDS( const std::vector<SHAPE*>& aShapes )
{
...
int DRAWING_TOOL::DrawOutline( const TOOL_EVENT& aEvent )
{
    ...
    while( OPT_TOOL_EVENT evt = Wait() )
    {
        ...
        if( needRefresh )
        {
            ...
            if ( outlineBuilder.Construct ( shapes ) )
            {
                dsegs = convertShapesToDS( shapes );
                preview.Clear();
...
int DRAWING_TOOL::DrawLinearConstraint( const TOOL_EVENT& aEvent )
{
    ...
    while( OPT_TOOL_EVENT evt = Wait() )
    {
        ...
        else if( evt->IsAction( &linearConstraintOrientation ) )
        {
            switch( orientAngle )
            {
...
void DRAWING_TOOL::runPolygonEventLoop( POLYGON_GEOM_MANAGER& polyGeomMgr )
{
    ...
    outlineBuilder->SetAllowedShapeTypes ( { OUTLINE_SHAPE_BUILDER::SHT_LINE, OUTLINE_SHAPE_BUILDER::SHT_CORNER_90, OUTLINE_SHAPE_BUILDER::SHT_CORNER_45 } );
