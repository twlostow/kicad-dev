...
#if 0
class LIB_TABLE_NODE;
WX_DEFINE_ARRAY_PTR( LIB_TABLE_NODE*, LIB_TABLE_NODE_ARRAY );
...
{
public:
    LIB_TABLE_NODE( LIB_TABLE_NODE* parent,
            wxPath aLibraryPath,
            LIB_SCOPE_T aScope,
            bool aEnabled,
            wxString aType
...
private:
    LIB_TABLE_NODE* m_parent;
    LIB_TABLE_NODE_ARRAY m_children;
};

class LIB_TABLE_MODEL : public wxDataViewModel
{
public:
    LIB_TABLE_MODEL()
    {
    ...
    void Add( NETCLASSPTR aNetclass, const COLOR4D& aDefaultColor  )
    {
        wxASSERT( m_root );

        auto child_node =
            new NET_COLOR_NODE( m_classes,
                    aNetclass->RatsnestPrefs(),
                    aDefaultColor,
                    aNetclass->GetName() );
        m_classes->Append( child_node );
    }

    void Add( NETINFO_ITEM* aItem, const COLOR4D& aDefaultColor  )
    {
        wxASSERT( m_root );

        auto child_node =
            new NET_COLOR_NODE( m_nets,
                    aItem->RatsnestPrefs(),
                    aDefaultColor,
                    aItem->GetShortNetname() );
    ...
    DIALOG_NET_VISIBILITY_AND_COLORS_BASE( aParent )
{
    ...
    Connect( wxEVT_TEXT_ENTER,
            wxCommandEventHandler( DIALOG_NET_VISIBILITY_AND_COLORS::onOkClick ),
            NULL,
            this );
...
bool DIALOG_NET_VISIBILITY_AND_COLORS::TransferDataToWindow()
{
    ...
    auto defaultColor = m_frame->Settings().Colors().GetLayerColor( LAYER_RATSNEST );

    for( int i = 1; i<netinfo.GetNetCount(); i++ )
        m_model->Add( netinfo.GetNetItem( i ), defaultColor );

    const auto& classes = m_frame->GetBoard()->GetDesignSettings().m_NetClasses;

    for( auto cl : classes )
        m_model->Add( cl.second, defaultColor );
...
{
public:
    DIALOG_FP_LIB_TABLE( wxTopLevelWindow* aParent, FP_LIB_TABLE* aGlobal,
