...
int sloppy_vertex_wind(GtsVertex *a, GtsVertex *b, GtsVertex *c)
{
	return point_wind(GTS_POINT(a), GTS_POINT(b), GTS_POINT(c));
}
...
guint check_triangle_interior_capacity(GtsTriangle *t, toporouter_vertex_t *v, toporouter_vertex_t *curpoint,
...
triangle_candidate_points_from_vertex(GtsTriangle *t, toporouter_vertex_t *v, toporouter_vertex_t *dest, toporouter_route_t *routedata)
{
	...
	{
		if (TOPOROUTER_CONSTRAINT(op_e)->box->type == T_BOARD)
		{
#ifdef DEBUG_ROUTE
			printf("T_BOARD constraint\n");
#endif
			return NULL;
		}
		if (constraint_netlist(TOPOROUTER_CONSTRAINT(op_e)) != vertex_netlist(dest))
...
all_candidates_on_edge(toporouter_edge_t *e, toporouter_route_t *routedata)
{
	...
	else if (TOPOROUTER_CONSTRAINT(e)->box->type == T_BOARD)
	{
		return NULL;
	}
																		...
																		toporouter_route_t *routedata)
{
	...
	if (TOPOROUTER_IS_CONSTRAINT(e1))
	{
		...
		if (TOPOROUTER_CONSTRAINT(e1)->box->type == T_BOARD)
		{
			noe1 = 1;
		}
...
triangle_candidate_points_e2:
	...
	if (TOPOROUTER_IS_CONSTRAINT(e2))
	{
		...
		if (TOPOROUTER_CONSTRAINT(e2)->box->type == T_BOARD)
		{
			noe2 = 1;
			//      goto triangle_candidate_points_finish;
		}
												 ...
												 toporouter_vertex_t **closestdest)
{
	...
	if ((tempedge = TOPOROUTER_EDGE(gts_vertices_are_connected(GTS_VERTEX(curpoint), GTS_VERTEX(*closestdest)))))
	{
		...
		else
		{
			if (!tempedge->routing)
			{
				result = g_list_prepend(NULL, *closestdest);
	...
	if (!(curpoint->flags & VERTEX_FLAG_TEMP))
	{
		...
		while (i)
		{
			...
			result = g_list_concat(result, temppoints);
			i = i->next;
		}
	...
	else /* a temp point */
	{
		...
		while (i)
		{
			...
			if (prevwind != vertex_wind(GTS_SEGMENT(edge)->v1, GTS_SEGMENT(edge)->v2, oppv))
			{
				...
				if (tr->flags & TOPOROUTER_FLAG_LEASTINVALID)
					temppoints = triangle_all_candidate_points_from_edge(tr, GTS_TRIANGLE(i->data), edge,
...
compute_candidate_points_finish:
	...
	if (vertex_bbox(curpoint) && vertex_bbox(curpoint)->cluster)
	{
		...
		{
			result = g_list_concat(result, g_list_copy(data->srcvertices));
		}
	}
			...
			toporouter_netlist_t *pair)
{
	...
	*n = pv->gn;
	...
	if (pair && !TOPOROUTER_IS_CONSTRAINT(v->routingedge) && v->routingedge)
	{
		...
		toporouter_vertex_t *prevV = edge_routing_prev_not_temp(v->routingedge, list);
		toporouter_vertex_t *nextV = edge_routing_next_not_temp(v->routingedge, list);

		if (prevV->route && prevV->route->netlist == pair)
		{
		}
		else if (nextV->route && nextV->route->netlist == pair)
		{
		}
		else
		{
...
GList *
route_find_path(toporouter_t *r, toporouter_route_t *data, guint debug)
{
	...
	printf("route %p destv-len %d srcv-len %d cp %p [%.0f %.0f] destv %p [%.0f %.0f]\n", data, g_list_length( data->destvertices ), g_list_length( data->srcvertices ), curpoint, vx(curpoint), vy(curpoint), destv, vx(destv), vy(destv) );
	...
	//printf("cur layer %p dest_layer %p netlist %p\n", cur_layer, dest_layer, data->netlist  );
	...
	{
		GList *ilist = r->routednets;
		while (ilist)
		{
			toporouter_route_t *curroute = TOPOROUTER_ROUTE(ilist->data);
...
*/
	...
	{
		toporouter_vertex_t *pv = NULL;
		GList *ilist = data->path;
		while (ilist)
		{
			toporouter_vertex_t *tv = TOPOROUTER_VERTEX(ilist->data);

			if (pv && g_list_find(data->srcvertices, tv))
			{
				GList *temp = g_list_copy(ilist);
				g_list_free(data->path);
				data->path = temp;
				ilist = data->path;
			}
...

	{
		...
		while (ilist)
		{
			v = TOPOROUTER_VERTEX(ilist->data);
	...
	// /*
	{
		...
		while (ilist)
		{
			toporouter_vertex_t *v = TOPOROUTER_VERTEX(ilist->data);
...
toporouter_serpintine_new(gdouble x, gdouble y, gdouble x0, gdouble y0, gdouble x1, gdouble y1, gpointer start, gdouble halfa, gdouble radius, guint nhalfcycles)
{
													 ...
													 toporouter_vertex_t *opv, gint wind, gint *arcwind, gdouble *arcr, guint debug)
{
	...
	{
		gdouble dist = tvdistance(tedge_v1(pathv->routingedge), tedge_v2(pathv->routingedge)) / 2.;
...
//    );
		...
		{ /* edge is on one side of the segment */
...
netscore_create(toporouter_t *r, toporouter_route_t *routedata, guint n, guint id)
{
	...
	GList *path = route_find_path(r, routedata, 0);
	...
	printf("detour score %.1f rtd score %.1f [%p]\n", routedata->detourscore, routedata->score, routedata);
...
//#ifdef DEBUG_NETSCORES
		printf("%4d %15f %15f %15d %p\n", (*i)->id, (*i)->score, (*i)->pairwise_detour_sum, (*i)->pairwise_fails, *i);
//#endif
	}
...
void netscore_pairwise_calculation(toporouter_netscore_t *netscore, GPtrArray *netscores)
{
	...
	for (toporouter_netscore_t **i = netscores_base; i < netscores_base + netscores->len; i++)
	{
		...
		if (!netscore->pairwise_nodetour[i - netscores_base] && *i != netscore && (*i)->routedata->netlist != netscore->routedata->netlist)
		{
			...
			route_find_path(netscore->r, temproutedata, 0);
			...
			else if (!isfinite(temproutedata->score))
			{
...
gint netscore_pairwise_size_compare(toporouter_netscore_t **a, toporouter_netscore_t **b)
{
	...
	if (isfinite((*a)->score) && !isfinite((*b)->score))
		return -1;
	if (isfinite((*b)->score) && !isfinite((*a)->score))
		return 1;
...
guint order_nets_preroute_greedy(toporouter_t *r, GList *nets, GList **rnets)
{
	...
	printf("order_preroute_greedy: len %d\n", len );
	GPtrArray *netscores = g_ptr_array_sized_new(len);
	printf("order_preroute_greedy: ns len %d\n", netscores->len );
	
	guint failcount = 0;

	while (nets)
	{
		auto route = TOPOROUTER_ROUTE(nets->data);
		printf("- proc route %p score %.1f det %.1f\n", route, route->score, route->detourscore );
		toporouter_netscore_t *ns = netscore_create(r, route, len, failcount++);
		printf("- proc2 route %p score %.1f det %.1f\n", route, route->score, route->detourscore );
		if (ns)
		{
			printf("ns %p\n", ns);
...
gint roar_route(toporouter_t *r, toporouter_route_t *routedata, gint threshold)
{
	...
	if (route_find_path(r, routedata, 0))
	{
		GList *conflicts, *j;
		...
		while (j)
		{
			...
			if (route_find_path(r, conflict, 0))
			{
...
void detour_router(toporouter_t *r)
{
	...
	for (toporouter_route_t **i_route = (toporouter_route_t **)scores->pdata; i_route < (toporouter_route_t **)scores->pdata + scores->len; i_route++)
	{
		toporouter_route_t *curroute = (*i_route);
