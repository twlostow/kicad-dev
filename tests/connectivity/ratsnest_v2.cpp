...
void RN_NET::removeNode( RN_NODE_PTR& aNode, shared_ptr<CN_CLUSTER> aParent )
{
    //aNode->RemoveParent( aParent.get() );
...
void RN_NET::removeEdge( RN_EDGE_MST_PTR& aEdge, shared_ptr<CN_CLUSTER> aParent )
{
    // Save nodes, so they can be cleared later
    RN_NODE_PTR start = aEdge->GetSourceNode();
    RN_NODE_PTR end = aEdge->GetTargetNode();

    //start->RemoveParent( aParent.get() );
    //end->RemoveParent( aParent.get() );
...
{
    const auto anchors = aCluster->GetAnchors();

    if(anchors.size() == 1)
    {
        m_links.AddNode( anchors[0].x, anchors[0].y );
        return;
    }

    auto first = m_links.AddNode( anchors[0].x, anchors[0].y );
    ...
    for(int i = 1; i < anchors.size(); i++)
    {
        ...
        if ( end != first )
            m_links.AddConnection( first, end );
    }
}
