...
EDA_DRAW_PANEL_GAL_NG::EDA_DRAW_PANEL_GAL_NG( wxWindow* aParentWindow, wxWindowID aWindowId,
                                        const wxPoint& aPosition, const wxSize& aSize,
                                        GAL_TYPE aGalType, KIGFX::VIEW_BASE *aView ) :
    wxScrolledCanvas( aParentWindow, aWindowId, aPosition, aSize )
{
    ...
    m_view = aView;
    m_painter = m_view->GetPainter();
    printf("m_gal %p\n", m_gal);
    m_view->SetGAL( m_gal );
    m_painter->SetGAL ( m_gal );

    Connect( wxEVT_SIZE, wxSizeEventHandler( EDA_DRAW_PANEL_GAL_NG::onSize ), NULL, this );
    Connect( wxEVT_ENTER_WINDOW, wxEventHandler( EDA_DRAW_PANEL_GAL_NG::onEnter ), NULL, this );
    Connect( wxEVT_KILL_FOCUS, wxFocusEventHandler( EDA_DRAW_PANEL_GAL_NG::onLostFocus ), NULL, this );
    ...
    {
        Connect( eventType, wxEventHandler( EDA_DRAW_PANEL_GAL_NG::onEvent ),
    ...
    Connect( wxEVT_TIMER, wxTimerEventHandler( EDA_DRAW_PANEL_GAL_NG::onRefreshTimer ), NULL, this );
}


EDA_DRAW_PANEL_GAL_NG::~EDA_DRAW_PANEL_GAL_NG()
{
    delete m_painter;
...
void EDA_DRAW_PANEL_GAL_NG::SetFocus()
{
...
void EDA_DRAW_PANEL_GAL_NG::onPaint( wxPaintEvent& WXUNUSED( aEvent ) )
{
    m_pendingRefresh = false;
...
void EDA_DRAW_PANEL_GAL_NG::onSize( wxSizeEvent& aEvent )
{
...
void EDA_DRAW_PANEL_GAL_NG::Refresh( bool aEraseBackground, const wxRect* aRect )
{
    if( m_pendingRefresh )
        return;
...
void EDA_DRAW_PANEL_GAL_NG::ForceRefresh()
{
...
void EDA_DRAW_PANEL_GAL_NG::SetEventDispatcher( TOOL_DISPATCHER* aEventDispatcher )
{
...
void EDA_DRAW_PANEL_GAL_NG::StartDrawing()
{
...
void EDA_DRAW_PANEL_GAL_NG::StopDrawing()
{
    m_drawingEnabled = false;
    Disconnect( wxEVT_PAINT, wxPaintEventHandler( EDA_DRAW_PANEL_GAL_NG::onPaint ), NULL, this );
    m_pendingRefresh = false;
    m_drawing = true;
    m_refreshTimer.Stop();
}
...
void EDA_DRAW_PANEL_GAL_NG::SetHighContrastLayer( LAYER_ID aLayer )
{
...
void EDA_DRAW_PANEL_GAL_NG::SetTopLayer( LAYER_ID aLayer )
{
...
double EDA_DRAW_PANEL_GAL_NG::GetLegacyZoom() const
{
...
bool EDA_DRAW_PANEL_GAL_NG::SwitchBackend( GAL_TYPE aGalType )
{
...
void EDA_DRAW_PANEL_GAL_NG::onEvent( wxEvent& aEvent )
{
    if( m_lostFocus )
        SetFocus();
...
void EDA_DRAW_PANEL_GAL_NG::onEnter( wxEvent& aEvent )
{
...
void EDA_DRAW_PANEL_GAL_NG::onLostFocus( wxFocusEvent& aEvent )
{
    m_lostFocus = true;
...
void EDA_DRAW_PANEL_GAL_NG::onRefreshTimer( wxTimerEvent& aEvent )
{
    if( !m_drawingEnabled )
    {
        if( m_gal->IsInitialized() )
        {
            m_drawing = false;
            m_pendingRefresh = true;
            Connect( wxEVT_PAINT, wxPaintEventHandler( EDA_DRAW_PANEL_GAL_NG::onPaint ), NULL, this );
