...
public:
    VIEW_CACHE_ENTRY() :
            ...
            m_owner(NULL),
            //m_item (NULL),
            m_flags( VI_VISIBLE | VI_DIRTY | VI_CACHEABLE )
    {
...
public:
    ...
    VIEW_CACHE_ENTRY* Add( VIEW_ITEM_NG *aItem )
    {
...
/*  void SetVisible( VIEW_ITEM_NG *aItem, bool aVisible )
  ...
  void SetDirty( VIEW_ITEM_NG *aItem, bool aDirty )
  {
    ...
    {
        for ( boost::unordered_map<VIEW_ITEM_NG*, VIEW_CACHE_ENTRY*>::iterator i = m_cache.begin(); i != m_cache.end(); ++i )
        {
